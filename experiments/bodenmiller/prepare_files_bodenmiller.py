...
def prepare_files():
    ...
    with open(
            os.path.join(args.input_dir, args.inhibitor_used, fcs_save_file),
            "w") as f:
        f.write(json.dumps(files_to_save))

    with open(
            os.path.join(args.input_dir, args.inhibitor_used,
                         markers_save_file), "w") as f:
